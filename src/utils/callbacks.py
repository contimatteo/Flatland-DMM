...
class TrainIntervalLogger(Callback):
    ...
    def on_step_begin(self, step, logs):
        ...
        if self.step % self.interval == 0:
            if len(self.episode_rewards.get(agent, [])) > 0:
                ...
                print('{} episodes - episode_reward: {:.3f} [{:.3f}, {:.3f}]{}{}'.format(len(self.episode_rewards[agent]),
                                                                                         np.mean(self.episode_rewards[agent]),
                                                                                         np.min(self.episode_rewards[agent]),
                                                                                         np.max(self.episode_rewards[agent]),
                                                                                         formatted_metrics,
                                                                                         formatted_infos))
                print('')
            self.reset()
    ...
    def on_step_end(self, step, logs):
        ...
        if len(self.info_names) > 0:
            self.infos[agent] = self.infos.get(agent, []) + [[logs['info'][k] for k in self.info_names]]
