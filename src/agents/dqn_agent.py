...
class AbstractDQNAgent(Agent):
    ...
    def compute_batch_q_values(self, state_batch):
        batch = self.process_state_batch(state_batch)[0]
...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
        ...
        if hasattr(model.output, '__len__') and model.output.shape[0]:
            raise ValueError(
    ...
    def backward(self, reward_dict, terminal):
        ...
        if self.step % self.memory_interval == 0:
            ...
            if reward_dict==0.0: reward_dict = dict(enumerate([0 for _ in range(len(self.recent_observation))])) # bruttissimo
            ...
            for agent in range(len(self.recent_observation)):
                ...
                self.memory.append(obs, action, reward, terminal,
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            state0_batch = self.process_state_batch(state0_batch)[:,0,:]
