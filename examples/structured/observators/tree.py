...
class CustomTreeObservator(ObservationBuilder):
    """
    ...
    def get(self, handle: int = 0) -> Node:
        ...
        try:
            while queue:  # I stop when I raise MaxNodeMemory or I expanded all the tree
                ...
                # enumerating direction with respect to [left, forward, right, back]
                for i, branch_direction in enumerate([(orientation + i) % 4 for i in range(-1, 3)]):
                    ...
                    if possible_transitions[branch_direction]:
                        ...
                        node.childs[self.tree_explored_actions_char[i]] = node_observed
    ...
    def print_node_features(node: Node, label, indent):
        print(
            indent, "Direction ", label, ": ", node.dist_own_target_encountered, ", ",
            ...
            node.dist_to_next_branch, ", ", node.dist_min_to_target, ", ",
            node.num_agents_same_direction, ", ", node.num_agents_opposite_direction, ", ",
